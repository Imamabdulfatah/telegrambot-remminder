...
REMINDER_CHAT_IDS: Final = ['1544942060', 'CHAT_ID_2', 'CHAT_ID_3']  # Ganti dengan ID chat atau ID grup Anda
...
async def send_reminder(bot, chat_id: str, message: Optional[str] = None, photo_path: Optional[str] = None):
    if message:
...
if __name__ == '__main__':
    ...
    # Jadwal pengingat berbeda untuk masing-masing ID chat
    scheduler.add_job(send_reminder, 'cron', hour=19, minute=0, args=[app.bot, '1544942060', '<b>Jangan lupa makan bakso di jam 20:20!</b>'])
    scheduler.add_job(send_reminder, 'cron', hour=19, minute=5, args=[app.bot, 'CHAT_ID_2', '<b>Ingat, rapat dimulai jam 21:00!</b>'])
    scheduler.add_job(send_reminder, 'cron', hour=19, minute=10, args=[app.bot, 'CHAT_ID_3', '<b>Selamat bergabung di grup kami!</b>'])

    # Penjadwal foto untuk masing-masing ID chat
    scheduler.add_job(send_reminder, 'cron', hour=16, minute=30, args=[app.bot, '1544942060', None, 'path/image/photo1.jpeg'])
    scheduler.add_job(send_reminder, 'cron', hour=16, minute=35, args=[app.bot, 'CHAT_ID_2', None, 'path/image/photo2.jpeg'])
    scheduler.add_job(send_reminder, 'cron', hour=16, minute=40, args=[app.bot, 'CHAT_ID_3', None, 'path/image/photo3.jpeg'])
