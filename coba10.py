...
REMINDER_CHAT_ID2: Final = '6848795446'
REMINDER_CHAT_ID3: Final = '1544942060'
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        '<p><b>Selamat Datang di Pulcher! </b></p><p>Kami sangat senang Anda bergabung dengan kami di Pulcher. Anda telah masuk ke bot reminder kami dengan saya PulcherBot saya akan mengingatkan anda untuk menggunakan skincare secara rutin</p><p>  silakan hubungi tim support kami jika anda sudah memulai PulcherBot untuk dikasih reminder sesuai kondisi kulit anda</p> <p>Terima kasih atas kepercayaan Anda!</p>', 
        parse_mode='HTML'
    )
...
async def send_reminder(bot, chat_id, message=None, photo_path=None, with_buttons=False):
    if with_buttons:
        buttons = [
            [InlineKeyboardButton("Ya", callback_data=f"yes|{message}"), InlineKeyboardButton("Tidak", callback_data=f"no|{message}")]
        ]
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    choice, reminder_message = query.data.split('|')

    user_id = query.from_user.id
    user_name = query.from_user.username or query.from_user.first_name

    response_text = f'{user_id},{user_name},{reminder_message},{choice},{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\n'
    with open(RESPONSE_DATA_FILE, 'a') as file:
        file.write(response_text)

    if choice == "yes":
        response = "Terimakasih sudah mengikuti reminder kami, selangkah menuju kulit yang sehat"
    elif choice == "no":
        response = "Yuk lebih rutin lagi menjaga kulit anda, untuk hasil yang maksimal"
    else:
...
if __name__ == '__main__':
    ...
    scheduler.add_job(send_reminder, 'cron', hour=20, minute=49, args=[app.bot, REMINDER_CHAT_ID, '<b>Jangan lupa makan bakso di jam 20:20!</b>', None, True])
    scheduler.add_job(send_reminder, 'cron', hour=20, minute=50, args=[app.bot, REMINDER_CHAT_ID2, '<b>Selamat bergabung!</b>', None, True])
    scheduler.add_job(send_reminder, 'cron', hour=20, minute=52, args=[app.bot, REMINDER_CHAT_ID3, '<b>Sampai jumpa!</b>', None, True])
